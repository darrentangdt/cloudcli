 #!/bin/sh
#************************************************
# 文件名：SECAUD_LINUX_NOOWNER_RES.sh             
# 策略管理方：风险管理处安全技术群组            
# 脚本撰写方：生产办云平台项目组                               
# 日  期：2014年3月10日                             
# 功  能：检查无主文件  
...
v_logfile="SECAUD_LINUX_NOOWNER_RES.out"
> $v_logfile

[ -f  Conviction.out ] && rm -f  Conviction.out
sh ${v_golbalpath}/ACL/SECAUD_LINUX_NOOWNER_TM.sh &

#检查小于500G的文件系统
v_files=$(df -k|grep %|grep -iv Used|awk '{if ($(NF-4)<500000000) print $NF}')
...
if [ -f Conviction.out ]; then
	if [ $line -gt 0 ]; then
		echo 发现系统存在无主文件，信息过多，仅列举前10行，参考如下:>>$v_logfile
		head -10 Conviction.out >> $v_logfile
		echo "Non-Compliant"
		echo "不合规" >> $v_logfile
	else
		echo "Compliant"
