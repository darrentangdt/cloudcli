#!/bin/sh
#************************************************
# 文件名：SECAUD_HPUX_SYSCONFACL_RES.sh             
# 策略管理方：风险管理处安全技术群组            
# 脚本撰写方：生产办云平台项目组                               
# 日  期：2014年3月10日                                 
# 功  能：检查系统配置文件权限                         
...
echo 原始信息>>SECAUD_HPUX_SYSCONFACL_RES.out
echo ----->>SECAUD_HPUX_SYSCONFACL_RES.out
echo 权限及属主信息:>>SECAUD_HPUX_SYSCONFACL_RES.out
...
echo ----->>SECAUD_HPUX_SYSCONFACL_RES.out
echo 中间信息>>SECAUD_HPUX_SYSCONFACL_RES.out
echo ----->>SECAUD_HPUX_SYSCONFACL_RES.out

echo 以下文件权限、属主不合规 >>SECAUD_HPUX_SYSCONFACL_RES.out
#列出不合规的单个文件
...
echo ----->>SECAUD_HPUX_SYSCONFACL_RES.out
echo 最终结论>>SECAUD_HPUX_SYSCONFACL_RES.out
echo ----->>SECAUD_HPUX_SYSCONFACL_RES.out
...
if [ -f Conviction.out ]; then
	if [ $line -gt 0 ]; then
		echo "Non-Compliant"
		echo "不合规" >> SECAUD_HPUX_SYSCONFACL_RES.out
	else
		echo "Compliant"
		echo "合规" >> SECAUD_HPUX_SYSCONFACL_RES.out
	fi
else 
	echo "Compliant"
	echo "合规" >> SECAUD_HPUX_SYSCONFACL_RES.out
