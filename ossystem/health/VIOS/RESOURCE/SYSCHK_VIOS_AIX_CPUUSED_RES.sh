#!/bin/sh
#************************************************#
# 文件名:SYSCHK_VIOS_AIX_CPUUSED_RES.sh              #
# 作  者:CCSD_YOUTONGLI                            #
# 日  期:2010年 1月4 日                         #
# 功  能:检查cpu利用率                          #
# 复核人:                                       #
...
v_p1=`grep "V_AIX_HEA_CPUUSED" /home/ap/opscloud/health_check/VIOS/AIX_HEA_PARA.txt |awk -F= '{print $2}'`
...
if [ $v_p2 -gt $v_p1 ]; then
   ...
   if [ $v_p2 -gt $v_p1 ]; then
       ...
       if [ $v_p2 -gt $v_p1 ]; then
          echo "Non-Compliant"
          echo "异常,当前cpu 利用率为[$v_p2],超过阀值[$v_p1]。" > SYSCHK_VIOS_AIX_CPUUSED_RES.out
       else
          echo "Compliant"
          echo "正常" > SYSCHK_VIOS_AIX_CPUUSED_RES.out
       fi
   else
      echo "Compliant"
      echo "正常" > SYSCHK_VIOS_AIX_CPUUSED_RES.out
