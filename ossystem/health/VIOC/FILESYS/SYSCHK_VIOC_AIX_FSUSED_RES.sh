#!/bin/sh
#************************************************#
# 文件名:SYSCHK_VIOC_AIX_FSUSED_RES.sh           #
# 作  者:CCSD_LIYU                               #
# 日  期:2012年 12月25日                         #
# 功  能:检查文件系统利用率                      #
# 复核人:                                        #
...
#参数文件位置
v_parameter_file="/home/ap/opscloud/health_check/VIOC/AIX_HEA_PARA.txt"

#日志文件输出名字
v_logfile="SYSCHK_VIOC_AIX_FSUSED_RES.out"
> $v_logfile
...
if [ -s "$v_logfile" ];then
    echo "Non-Compliant"
	echo "异常,有使用率超出阈值的文件系统,请检查" >> $v_logfile
else
    echo "Compliant"
	echo "正常" >> $v_logfile
	df >> $v_logfile
